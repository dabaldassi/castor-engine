...
void touch(Element *th, int button){
    (void)button;
    
    nextAnimationElement(th);
    setFlipStateElement(th,SANDAL2_FLIP_NONE);
}

void endJump(Element *th,int code){
    if(code==1){
	setAnimationElement(th,0);
	setFlipStateElement(th,SANDAL2_FLIP_VER);
    }
}
...
int main(){
    ...
    // setRotationPointElement(objet,1.f);
